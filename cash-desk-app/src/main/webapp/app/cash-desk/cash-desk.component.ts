...
export class CashDeskComponent implements OnInit {
    ...
    enterItemIdentifier() {
        this.cashDeskService.verifyProductStock(this.barcode.toString()).subscribe(
            result => {
                if (result.body) {
    ...
    scanItem() {
        ...
        this.cashDeskService.verifyProductStock(barcode.toString()).subscribe(
            result => {
                if (result.body) {
    ...
    getProduct(barcode) {
        this.cashDeskService.getProduct(barcode).subscribe(
            result => {
                if (result.body) {
                    this.products.push(result.body);
                    this.calculateTotal();
                } else this.getProductResult = "product doesn't exist";
            },
            error => {
