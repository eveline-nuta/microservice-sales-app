...
class Multiple500TwoMins extends Simulation {
  ...
  setUp(scn1.inject(constantUsersPerSec(500) during (120)).protocols(httpProtocol1),
  scn2.inject(constantUsersPerSec(500) during (120)).protocols(httpProtocol2),
  scn3.inject(constantUsersPerSec(500) during (120)).protocols(httpProtocol3),
  scn4.inject(constantUsersPerSec(500) during (120)).protocols(httpProtocol4)
        )
}
