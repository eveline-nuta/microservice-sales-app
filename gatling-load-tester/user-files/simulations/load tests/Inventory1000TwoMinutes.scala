...
class Inventory1000TwoMinutes extends Simulation {
  ...
  setUp(scn1.inject(constantUsersPerSec(1000) during (120)).protocols(httpProtocol1),
  scn2.inject(constantUsersPerSec(1000) during (120)).protocols(httpProtocol2)
        )
}