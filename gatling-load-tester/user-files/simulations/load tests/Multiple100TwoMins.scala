...
class Multiple100TwoMins extends Simulation {
  ...
  scn3.inject(constantUsersPerSec(100) during (120)).protocols(httpProtocol3),
  scn4.inject(constantUsersPerSec(100) during (120)).protocols(httpProtocol4)
        )
}
