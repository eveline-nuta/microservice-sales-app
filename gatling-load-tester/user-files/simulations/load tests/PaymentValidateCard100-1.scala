...
class ValidateCardOneHundred extends Simulation {
  val httpProtocol1 = http.baseURL("http://localhost:8080").inferHtmlResources()
  val scn1 = scenario("Monolith Payment Validate Card").repeat(100) { 
  ...
  val scn2 = scenario("Microservice Payment Validate Card").repeat(100) { 
